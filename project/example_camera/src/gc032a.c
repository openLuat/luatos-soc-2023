...
const camera_reg_t gc032a_reg_table[] =
{
    /*System*/
    ...
    {0x3c, 0x10}, //{0x3c,0x00}
    {0xfe, 0x00},
	{0x44, 0x03},	//透传到LCD能正常显示颜色需要
};

int gc032a_reg_init(int i2c_id)
{
	for(int i = 0; i < sizeof(gc032a_reg_table)/sizeof(camera_reg_t); i++)
	{
		if (luat_i2c_send(i2c_id, GC032A_I2C_ADDR, (void *)&gc032a_reg_table[i].reg, 2, 1))
		{
			LUAT_DEBUG_PRINT("write %d %x,%x failed", i, gc032a_reg_table[i].reg, gc032a_reg_table[i].data);
			return -1;
		}
	}
	return 0;
