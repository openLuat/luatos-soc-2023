                           ...
                           unsigned int keybits )
{
    ...
    if( aes_padlock_ace == -1 )
        aes_padlock_ace = aes_padlock_has_support( INFRA_PADLOCK_ACE );
    ...
    if( aesni_has_support( INFRA_AESNI_AES ) )
        return( aesni_setkey_enc( (unsigned char *) ctx->rk, key, keybits ) );
#endif
...
static int aes_internal_aes_encrypt( aes_context *ctx,
...
static int aes_internal_aes_decrypt( aes_context *ctx,
                                     const unsigned char input[16],
                                     unsigned char output[16] )
{
    int i;
                          ...
                          unsigned char output[16] )
{
    ...
    if( aes_padlock_ace )
    {
        if( aes_padlock_xcryptecb( ctx, mode, input, output ) == 0 )
            return( 0 );
    ...
    if( mode == INFRA_AES_ENCRYPT )
        return( aes_internal_aes_encrypt( ctx, input, output ) );
    else
        return( aes_internal_aes_decrypt( ctx, input, output ) );
}
                          ...
                          unsigned char *output )
{
    ...
    if( aes_padlock_ace )
    {
        if( aes_padlock_xcryptcbc( ctx, mode, length, iv, input, output ) == 0 )
            return( 0 );
                             ...
                             unsigned char *output )
{
    ...
    if( mode == INFRA_AES_DECRYPT )
    {
        ...
        {
            if( n == 0 )
                aes_crypt_ecb( ctx, INFRA_AES_ENCRYPT, iv, iv );
...
#if defined(INFRA_AES_BUILDIN)
    aes_init(&p_aes128->ctx);
    if (dir == AES_ENCRYPTION) {
        ret = aes_setkey_enc(&p_aes128->ctx, key, 128);
    } else {
        ret = aes_setkey_dec(&p_aes128->ctx, key, 128);
    }
#else
...
static int aes128_destroy(void * aes)
{
    if (!aes) return -1;


#if defined(INFRA_AES_BUILDIN)
    aes_free(&((platform_aes_t *)aes)->ctx);
#else
...
#if defined(INFRA_AES_BUILDIN)
        ret = aes_crypt_cbc(&p_aes128->ctx, INFRA_AES_DECRYPT, AES_BLOCK_SIZE,
                            p_aes128->iv, src, dst);
#else
    ...
    void *dst)
{
    ...
    for (i = 0; i < blockNum; ++i) {
#if defined(INFRA_AES_BUILDIN)
        ret = aes_crypt_cbc(&p_aes128->ctx, INFRA_AES_ENCRYPT, AES_BLOCK_SIZE,
                            p_aes128->iv, src, dst);
#else
...
int iotx_aes_cbc_encrypt(const unsigned char *src, int len, const unsigned char *key, void *out)
{
    ...
    void * aes_e_h = aes128_init((unsigned char *)key, (unsigned char *)iv, AES_ENCRYPTION);
    if (len1) {
        ret = aes128_cbc_encrypt(aes_e_h, src, len1 >> 4, out);
    }
    if (!ret && pad) {
        char buf[16] = {0};
        memcpy(buf, src + len1, len - len1);
        memset(buf + len - len1, pad, pad);
        ret = aes128_cbc_encrypt(aes_e_h, buf, 1, (unsigned char *)out + len1);

    }

    aes128_destroy(aes_e_h);
...
int iotx_aes_cbc_decrypt(const unsigned char *src, int len, const unsigned char *key, void *out)
{
    ...
    aes_d_h  = aes128_init((uint8_t *)key, (uint8_t *)iv, AES_DECRYPTION);
    if (!aes_d_h) {
        // COAP_INFO("fail to decrypt");
        return  0;
    }
    if (n > 1) {
        ret = aes128_cbc_decrypt(aes_d_h, src, n - 1, out);
    }
    ...
    if (ret == 0) {
        ...
        if (aes_d_h) {
            ret = aes128_cbc_decrypt(aes_d_h, src + offset, 1, out_c + offset);
        } else {
